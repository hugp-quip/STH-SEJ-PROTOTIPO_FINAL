...
func comecar(_baralhoAT: BarRES, n_rodadas: int) -> void:
	assert(_baralhoAT.cartas.size()/5 >= n_rodadas, "BARALHO MUITO PEQUENO PARA O NÚMERO DE RODADAS REQUISITADO!!!")
...
func _get_hand(n_cartas : int) -> Array:
	...
	while cartasID.size() < n_cartas:
		var r : int = random.randi_range(0, cartas.size()-1) 
