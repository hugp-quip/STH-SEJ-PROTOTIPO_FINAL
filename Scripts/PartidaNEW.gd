...
var rodada : Node 
var alb : Node = load("res://Scenes/album.tscn").instantiate()
...
func on_rodadaTerminada(resultado) -> void:
	...
	if resultado == "derrota": 
		if Pontuação not in G.rank.performances:
			G.rank.performances.append(Pontuação)	
...
func get_maiorPontuação() -> int:
	return G.rank.performances[-1]
	
'''
var ma = 0
for performance in G.rank.performances:
	if performance > ma:
		ma = performance
return ma
'''
...
func _on_see_alb_pressed() -> void:
	var b = alb.get_node("Voltar") 
	b.pressed.disconnect(alb._on_voltar_pressed)
	b.pressed.connect(leave_alb)
	get_parent().add_child(alb)
	alb.show()
	#visible = false # the node2Ds will have to be made invisible somehow TWT
	
func leave_alb() -> void:
	alb.hide()
	get_parent().remove_child(alb)
	#visible = true
