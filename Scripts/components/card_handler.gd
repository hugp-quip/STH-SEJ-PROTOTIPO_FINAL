...
func _ready() -> void:
	...
	cards_dragged = get_children().map( func (child) : if child is TempCartaDisplay : return child).filter( func(child) : return child != null)
...
# 	return get_node("Storage_Position").position

# func set_storage_position() -> void:
# 	storage_position = get_storage_position()
